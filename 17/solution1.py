...
field = np.expand_dims(np.array(input[:]), 0)
...
def advanceConway(field):
    ...
    newField = np.full((len(paddedField), len(paddedField[0]), len(paddedField[0][0])),".")
    ...
    for i in range(len(newField)):
        for j in range(len(newField[0])):
            for k in range(len(newField[0][0])):
                activeNeighors = checkNeighbors(paddedField, i, j, k)
                if paddedField[i][j][k] == ".":
                    if activeNeighors == 3:
                        newField[i][j][k] = "#"
                    else:
                        newField[i][j][k] = "."
...
def checkNeighbors(field, x, y, z):
    total = 0
...
print(field.shape)
activeCells = np.sum(np.sum(np.sum((field == "#"),axis=0),axis=0),axis=0)
print(activeCells)